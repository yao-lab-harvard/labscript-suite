...
class PulseBlaster(PseudoclockDevice):
    ...
    def __init__(self, name, trigger_device=None, trigger_connection=None, board_number=0, firmware = '',
                 ...
                 time_based_stop_workaround=False, time_based_stop_workaround_extra_time=0.5,  **kwargs): #ejd loop_number=1e5, 
    ...
    #              time_based_stop_workaround=False, time_based_stop_workaround_extra_time=0.5,  **kwargs):
        ...
        # that.
        # self.clock_limit = .5e6
        self.min_delay = 0.5 / self.clock_limit
        # print("Pulseblaster CLOCK LIMIT: ", self.clock_limit)
        # print("MIN DELAY: ", self.min_delay) #9/4/2023
        ...
        # Deal with loops
        self.loop_number = 1 #ejd
        self.outer_loop = False #ejd
        self.loopStarts = {}#ejd
        self.loopEnds = []#ejd
        # self.mod_loop_number = None
    ...
    #ejd
    def setParams(self, myMod):
        self.loop_number = myMod
        return
    def outerLoop(self, loop_number):
        if loop_number > 1:
            self.outer_loop = True #loop_number ejd 9/7/23
            self.loop_number = loop_number
        return

    def startLoop(self, t, loopName, numLoops): 
        loopType = loopName[0]
        if loopType not in ['outer', 'inner']:
            pass #raise #TODO add other checks etc
        if loopType+str(loopName[1:]) not in self.loopStarts: #create if doesn't exist
            self.loopStarts[loopType+str(loopName[1:])] = {'startTime': t, 'numLoops': numLoops, 'endTime':None, 'n':0, 'loopIndex': loopName[1:]}   
        return

    def endLoop(self, t, loopName):
        loopType = loopName[0]
        if loopType not in ['outer', 'inner']:
            pass #raise #TODO add other checks etc.
        if self.loopStarts[loopType+str(loopName[1:])]['endTime'] == None: #create if doesn't exist
            self.loopStarts[loopType+str(loopName[1:])]['endTime'] = t
        return
    ...
    def convert_to_pb_inst(self, dig_outputs, dds_outputs, freqs, amps, phases):
        pb_inst = []
        loop_bool = False
        # check whether the user has indicated that there are loops in the sequence 
        if len(self.loopStarts.keys()) != 0:
            loop_bool = True
        # get the stop time of the sequence
        sequence_end_time = self.pseudoclock.clock[-1]['start'] + self.pseudoclock.clock[-1]['step']
        print("sequence end time ", sequence_end_time)
        # print('GET END TIME', self.pseudoclock.clock[-1]['start'], self.pseudoclock.clock[-1]['step'])
        # print('GET END TIME', self.pseudoclock.clock[-2]['start'], self.pseudoclock.clock[-2]['step'])
        # print('GET END TIME', self.pseudoclock.clock[-3]['start'], self.pseudoclock.clock[-3]['step'])
        ...
        pb_inst.append({'freqs': freqregs, 'amps': ampregs, 'phases': phaseregs, 'enables':dds_enables, 'phase_resets': phase_resets,
                        ...
                        'data': 0, 'delay': 10.0/self.clock_limit*1e9, 'loops': []}) #ejd 8/14
        pb_inst.append({'freqs': freqregs, 'amps': ampregs, 'phases': phaseregs, 'enables':dds_enables, 'phase_resets': phase_resets,
                        'flags': ''.join([str(flag) for flag in flags]), 'instruction': 'STOP',
                        'data': 0, 'delay': 10.0/self.clock_limit*1e9, 'loops': []}) #ejd 8/14    
        j += 2
        ...
        for k, instruction in enumerate(self.pseudoclock.clock):
            pb_loops = []
            if loop_bool:
                for loop in self.loopStarts.keys():
                    startLoop = self.loopStarts[loop]['startTime'] 
                    endLoop = self.loopStarts[loop]['endTime'] 
                    numLoops = self.loopStarts[loop]['numLoops']
                    loopDur = endLoop - startLoop
                    n=self.loopStarts[loop]['n']
                    while endLoop + n * loopDur < sequence_end_time and n < numLoops:
                        if round(startLoop + n*loopDur, 12) <= round(instruction['start'], 12) < round(endLoop + n*loopDur, 12): #TODO: round or not??? ---> decided to round to nearest ps. 
                            #TODO complain if times are equal?
                            pb_loops.append((loop, n))
                            self.loopStarts[loop]['n'] = n
                            n=numLoops #there can't be more than one, so exit the while
                        n+=1
            ...
            if not only_internal:
                ...
                pb_inst.append({'freqs': freqregs, 'amps': ampregs, 'phases': phaseregs, 'enables':dds_enables, 'phase_resets':phase_resets,
                                ...
                                'data': instruction['reps'], 'delay': high_time*1e9, 'loops': pb_loops}) #ejd 8/14
                ...
                if n_long_delays:
                    pb_inst.append({'freqs': freqregs, 'amps': ampregs, 'phases': phaseregs, 'enables':dds_enables, 'phase_resets':phase_resets,
                                ...
                                'data': int(n_long_delays), 'delay': self.long_delay*1e9, 'loops': pb_loops}) #ejd 8/14
                                
                # Remaining low time. Clock edges are low:
                pb_inst.append({'freqs': freqregs, 'amps': ampregs, 'phases': phaseregs, 'enables':dds_enables, 'phase_resets':phase_resets,
                                'flags': flagstring, 'instruction': 'END_LOOP',
                                'data': j, 'delay': remaining_low_time*1e9, 'loops': pb_loops})
                ...
                if n_long_delays:
                    pb_inst.append({'freqs': freqregs, 'amps': ampregs, 'phases': phaseregs, 'enables':dds_enables, 'phase_resets':phase_resets,
                                'flags': flagstring, 'instruction': 'LONG_DELAY',
                                'data': int(n_long_delays), 'delay': self.long_delay*1e9, 'loops': pb_loops})

                pb_inst.append({'freqs': freqregs, 'amps': ampregs, 'phases': phaseregs, 'enables':dds_enables, 'phase_resets':phase_resets,
                                'flags': flagstring, 'instruction': 'CONTINUE',
                                'data': 0, 'delay': remaining_delay*1e9, 'loops': pb_loops})
                
                j += 2 if n_long_delays else 1
            
            if pb_inst[k]['delay'] <= 12:
                msg = """PulseBlaster instruction {0} has length < 12ns! \n
                Flags before inst {0}: {1} \n
                Flags of inst {0}: {2} \n
                Flags after inst {0}: {3}
                """
                warnings.warn(msg.format(k, pb_inst[k-1]['flags'], pb_inst[k]['flags'], pb_inst[k+1]['flags']))
        ...
        if self.programming_scheme == 'pb_start/BRANCH':
            ...
            pb_inst.append({'freqs': freqregs, 'amps': ampregs, 'phases': phaseregs, 'enables':dds_enables, 'phase_resets':phase_resets,
                            ...
                            'data': 0, 'delay': 10.0/self.clock_limit*1e9, 'loops': []})
        ...
        # print('\n\n\n BEFORE EDITS')
        # for i in range(len(pb_inst)):
        #     print(i, pb_inst[i])
        # print('\n\n\n')

        #############################
        ######## pb_inst processing v2
        #############################
        deleted_inst = [] # list of deleted instructions for correcting indices of END_LOOPs later
        inserted_inst = [] # list of inserted instructions for correcting indices of END_LOOPs later

        if self.outer_loop: 
            print('processing numIterations loop')
            pb_inst[2]['delay'] = 50
            pb_inst[3]['instruction'] = 'LOOP'
            pb_inst[3]['data'] = self.loop_number
            pb_inst[3]['delay'] = 2*pb_inst[3]['delay']

            pb_insert = pb_inst[-4].copy()
            pb_insert['instruction'] = 'END_LOOP'
            pb_insert['data'] = 3
            pb_insert['delay'] = pb_inst[-4]['delay']/2
            pb_inst[-4]['delay'] = pb_inst[-4]['delay']/2
            pb_inst.insert(-3, pb_insert)
            inserted_inst.append(len(pb_inst)-3)

        if loop_bool:
            ##### make inner and outer loop lists
            innerLoops = []
            outerLoops = []
            for j, loop in enumerate(self.loopStarts.keys()):
                if loop[:5] == 'inner': 
            ...
            print('processing inner loops...')
            for innerLoop in innerLoops:
                found = False
                foundInd = None
                for i in range(len(pb_inst)): #go through all pb_inst
                    if (innerLoop, 0) in pb_inst[i]['loops'] and not found: #this is the beginning of the first loop
                        found = True #beginning of loop is found
                        foundInd = i #index of beginning of loop

                        if pb_inst[i]['instruction'] == 'LOOP': #beginning of loop is already LOOP
                            if pb_inst[i]['data'] == 1: #if length of existing loop is 1, simply replace with numLoops and change END_LOOP to continue
                                print(i, 'INNER LOOP: insertion point has LOOP and n=1')
                                pb_inst[i]['data'] = self.loopStarts[innerLoop]['numLoops']
                                if pb_inst[i+1]['instruction'] == 'END_LOOP': #TODO this might not be right after loop...
                                    pb_inst[i+1]['instruction'] = 'CONTINUE'
                            
                            else: #insertion point has LOOP and n > 1 #TODO not implemented
                                print(i, 'INNER LOOP: insertion point has LOOP and n>1. WARNING: not implemented')
                                # TODO raise error while not implemented correctly
                                # save info to insert LOOP at END
                                # insert = pb_inst[i]
                                # insert['data'] = self.loopStarts[innerLoop]['numLoops']
                                # pb_insert_inst.append((i, insert))
                                # # decrement current LOOP
                                # pb_inst[i]['data'] = pb_inst[i]['data'] - 1

                        elif pb_inst[i]['instruction'] == 'CONTINUE': #beginning of loop is CONTINUE. Simply replace. 
                            pb_inst[i]['instruction'] = 'LOOP'
                            pb_inst[i]['data'] = self.loopStarts[innerLoop]['numLoops']
                        
                        else: #beginning of loop is END_LOOP, STOP, or BRANCH
                            print(i, 'INNER LOOP: insertion point END_LOOP, STOP or BRANCH. WARNING: not implemented')
                            pass #TODO raise error or implement correctly
                    
                    elif (innerLoop, 0) in pb_inst[i]['loops'] and found: #in first loop but not the beginning
                        if (innerLoop, 1) in pb_inst[i+1]['loops']: # check if end of first loop 
                            ###############################
                            ### DO END OF FIRST LOOP STUFF
                            ###############################
                            endLoopInd = i
                            if pb_inst[i]['instruction'] == 'END_LOOP': #if end of loop is already an END_LOOP
                                print(i, 'INNER LOOP: end of loop is already END_LOOP. WARNING: not implemented')
                                pass #TODO
                            elif pb_inst[i]['instruction'] == 'CONTINUE':
                                pb_inst[i]['instruction'] = 'END_LOOP'
                                pb_inst[i]['data'] = foundInd - len(np.array(deleted_inst)[np.array(deleted_inst) < foundInd])#len(deleted_inst[deleted_inst < foundInd])#deleteCount                                              
                            else: #end of loop is LOOP, STOP, or BRANCH
                                print(i, 'INNER LOOP: end of loop is LOOP, STOP, or BRANCH. WARNING: not implemented')
                                pass #TODO raise error or implement correctly
                        
                        else: #in first loop but not at end of first loop
                            if pb_inst[i]['instruction'] == 'END_LOOP': #if we are in the first loop and encounter END_LOOPs, we need to correct their data
                                currInd = pb_inst[i]['data']
                                deleted_inst = list(set(deleted_inst)) #get rid of any duplicate indices
                                pb_inst[i]['data'] = currInd - len(np.array(deleted_inst)[np.array(deleted_inst) < currInd])

                    elif len(pb_inst[i]['loops']) > 0 and innerLoop in np.array(pb_inst[i]['loops'])[:,0]: #not in first loop
                        pb_inst[i]['instruction'] += innerLoop + ' DELETE'
                        deleted_inst.append(i)
                        # deleted_inst = list(set(deleted_inst)) #get rid of duplicate indices --> moved this, it was very slow
                        ##########################################
                        #### check for EDGE CASE Y
                        #### instruction combines end of loop for inner loop and end of loop for outer loop 
                        ##########################################
                        outer = 'outer[' + str(self.loopStarts[innerLoop]['loopIndex'][0]) +']' #use user-provided loop structure to get the outerLoop for this innerLoop
                        #### check that it's the last instruction of this inner loop and the end of the outer loop
                        if (((innerLoop, self.loopStarts[innerLoop]['numLoops']-1) in pb_inst[i]['loops']) 
                            and ((outer, 0) in pb_inst[i]['loops']) 
                            and not ((innerLoop, self.loopStarts[innerLoop]['numLoops']-1) in pb_inst[i+1]['loops'])):
                            print(i, 'EDGE CASE Y')
                            #### Solution: the end instruction of the inner loop is already included in its own loop, so we don't want it
                            #### again here. Hence, remove the inner loop instruction but keep the outer loop instruction.  
                            pb_inst[i]['delay'] = pb_inst[i]['delay'] - pb_inst[endLoopInd]['delay']
            deleted_inst = list(set(deleted_inst)) #get rid of any duplicate indices
            
            
            print('processing outer loops...')
            for outerLoop in outerLoops:
                found = False
                foundInd = None
                for i in range(len(pb_inst)): #go through all pb_inst
                    if (outerLoop, 0) in pb_inst[i]['loops'] and not found: #this is the beginning of the first loop
                        found = True #beginning of loop is found
                        foundInd = i

                        if pb_inst[i]['instruction'] == 'LOOP': #beginning of loop is already a loop
                            if pb_inst[i]['data'] == 1: #if length of existing loop is 1, simply replace with numLoops and change END_LOOP to continue
                                print(i, 'OUTER LOOP: insertion point has LOOP and n=1')
                                pb_inst[i]['data'] = self.loopStarts[outerLoop]['numLoops']
                                if pb_inst[i+1]['instruction'] == 'END_LOOP': #TODO this might not be right after loop...
                                    pb_inst[i+1]['instruction'] = 'CONTINUE'
                            
                            else: #insertion point has LOOP and n > 1 #TODO not implemented
                                print(i, 'OUTER LOOP: insertion point has LOOP and n>1. WARNING: not implemented')
                                # TODO raise error while not implemented correctly
                                # save info to insert LOOP at END
                                # insert = pb_inst[i]
                                # insert['data'] = self.loopStarts[innerLoop]['numLoops']
                            ...
                            print(i, 'OUTER LOOP: insertion point is END_LOOP, STOP, or BRANCH. WARNING: not implemented')
                            pass #TODO raise error or implement correctly
                    
                    elif (outerLoop, 0) in pb_inst[i]['loops'] and found: #in first loop but not the beginning
                        if (outerLoop, 1) in pb_inst[i+1]['loops']: # check if end of first loop 
                            ###############################
                            ### DO END OF FIRST LOOP STUFF
                            ###############################
                            if 'END_LOOP' in pb_inst[i]['instruction']: #if end of loop is already an END_LOOP
                                end_loop_begin_index = pb_inst[i]['data'] # go back to beginning of this loop
                                if pb_inst[end_loop_begin_index]['data'] == 1: # if num loops ==1
                                    pb_inst[end_loop_begin_index]['instruction'] = 'CONTINUE' ##change LOOP to CONTINUE
                                    # for fix in range(index,racoon): #TODO put in
                                    #     pb_inst[fix]['instruction'].replace(' RACOON INNER DELETE',"") #TODO change number of main loops?
                                    pb_inst[i]['data'] = foundInd - len(np.array(deleted_inst)[np.array(deleted_inst) < foundInd])
                                else: #num loops is greater than 1
                                    print(i, 'OUTER LOOP: end of loop is already an END_LOOP and n>1. WARNING: not implemented')
                                    pass #TODO 

                            elif 'CONTINUE' in pb_inst[i]['instruction']: #end of loop is continue
                                pb_inst[i]['instruction'] = 'END_LOOP' #simply replace it
                                if i in deleted_inst: #if this instruction was previously marked for deletion while going through another loop, we are putting it back. Hence, remove from list of deleted instructions.
                                    deleted_inst.remove(i)
                                deleted_inst = list(set(deleted_inst)) #get rid of duplicate indices
                                pb_inst[i]['data'] = foundInd - len(np.array(deleted_inst)[np.array(deleted_inst) < foundInd])
                                #########################################
                                #### check for EDGE CASE Zo: beginning and end of loop got combined
                                #########################################
                                if pb_inst[foundInd]['instruction'] == 'LOOP': #TODO
                                    # check if next instruction = second (or third) instruction of 0th loop #TODO
                                    # also check if length of (outer, 0) = length (outer, 1) - 1 (or - 2) #TODO
                                    if round(pb_inst[i+1]['delay'], 3) == round(pb_inst[foundInd+2]['delay'], 3): #round to nearest ps
                                        pb_inst[i]['delay'] = pb_inst[i]['delay'] - 2*pb_inst[foundInd]['delay'] #TODO
                                        print('EDGE CASE Zo')

                            else: #end of loop is LOOP, STOP, or BRANCH
                                print(i, 'OUTER LOOP: end of loop is LOOP, STOP, or BRANCH. WARNING: not implemented')
                                pass #TODO raise error or implement correctly

                        else: #i is not the end of the first loop; we are inside first loop
                            if 'END_LOOP' in pb_inst[i]['instruction']: #if i is inside the first loop and we encounter END_LOOPs, we need to correct their data
                                currInd = pb_inst[i]['data']
                                deleted_inst = list(set(deleted_inst)) #get rid of duplicate indices
                                pb_inst[i]['data'] = currInd - len(np.array(deleted_inst)[np.array(deleted_inst) < currInd])


                    elif len(pb_inst[i]['loops']) > 0 and outerLoop in np.array(pb_inst[i]['loops'])[:,0]: #delete everything after first loop
                        pb_inst[i]['instruction'] += outerLoop + ' DELETE'
                        deleted_inst.append(i)
                        # deleted_inst = list(set(deleted_inst)) #get rid of duplicate indices --> moved this, it was very slow
            
            deleted_inst = list(set(deleted_inst)) #get rid of duplicate indices
            print('checking any remaining instructions not inside a loop...') # done checking loops. any remaining instructions:
            for i in range(len(pb_inst)): #fix data of any remaining END_LOOPs
                if len(pb_inst[i]['loops']) == 0 and 'END_LOOP' in pb_inst[i]['instruction']:
                    currInd = currInd = pb_inst[i]['data']
                    pb_inst[i]['data'] = currInd - len(np.array(deleted_inst)[np.array(deleted_inst) < currInd])
        ...
        pb_inst_final = []
        for i in range(len(pb_inst)):
            if not 'DELETE' in pb_inst[i]['instruction']:
                pb_inst_final.append(pb_inst[i])
        pb_inst_final[-2]['data'] = len(pb_inst_final) - 3 #TODO????
        # print('! ', len(pb_inst_final) - 3)
        # print('!! ', pb_inst_final[-2])
        pb_inst = pb_inst_final
...
#############################
        ...
        #     outerLoops = []
        #     for j, loop in enumerate(self.loopStarts.keys()):
        #         if loop[:5] == 'inner': #find all inner lists
        #             innerLoops.append(loop)
        #         elif loop[:5] == 'outer':
        #             outerLoops.append(loop)
            
        #     ###### go through inner loops and organize ... not currently making use of label structure TODO
        #     # TODO might need to change loop and end_loop at beginning and end especially if there is already a loop or end_loop there
        #     foundInd = 0
        #     deleteCount = 0
        #     deleteCountPrev = 0
        #     pb_insert_inst = []
        ...
        #         for i in range(len(pb_inst)): #go through all pb_inst
        #             # print(i, pb_inst[i])
        #             if innerLoop in pb_inst[i]['loops'] and not found: #change pb_inst at beginning of loop
        #                 found = True
        #                 loopLen+=1
        #                 foundInd = i
        #                 # print("FOUND! ", i, foundInd)
        #                 if pb_inst[i]['instruction'] == 'LOOP':
        #                     if pb_inst[i]['data'] == 1:
        #                         print(i, 'insertion point has LOOP and n=1')
        #                         pb_inst[i]['data'] = self.loopStarts[innerLoop]['numLoops']
        #                         if pb_inst[i+1]['instruction'] == 'END_LOOP':
        #                             pb_inst[i+1]['instruction'] = 'CONTINUE'
        #                     else: #insertion point has LOOP and n > 1 #TODO this is not implemented correctly
        #                         print(i, 'insertion point has LOOP and n>1')
        #                         # save info to insert LOOP at END
        #                         insert = pb_inst[i]
        #                         insert['data'] = self.loopStarts[innerLoop]['numLoops']
        ...
        #                     pb_inst[i]['instruction'] = 'LOOP'
        #                     pb_inst[i]['data'] = self.loopStarts[innerLoop]['numLoops']
        #             elif innerLoop in pb_inst[i]['loops'] and found: #going through rest of loop, but not the first instruction of the loop
        #                 loopLen += 1
        #                 # print("in loop ", i, loopLen)
        #             elif found and innerLoop not in pb_inst[i]['loops'] and foundInd < i <= foundInd + loopLen: #reached end of loop
        #                 # print('racoon', i)
        #                 if i == foundInd + loopLen: #this is the end of the loop 
        #                     racoon = i-1
        #                     # print("RACOON INNER", racoon, pb_inst[racoon-1], pb_inst[racoon], pb_inst[racoon+1])
        #                     if pb_inst[racoon]['instruction'] == 'END_LOOP': # if there is already an end_loop there...
        #                         # print("RACOON!")
        #                         # print("RACOON INNER", pb_inst[racoon])
        #                         index = pb_inst[racoon]['data'] # go back to beginning of this loop
        #                         if pb_inst[index]['data'] == 1: # if num loops ==1
        #                             pb_inst[index]['instruction'] = 'RACOON CONTINUE' #RACOON CONTINUE #change loop to continue
        #                             # for fix in range(index,racoon):
        #                             #     pb_inst[fix]['instruction'].replace(' RACOON INNER DELETE',"") #TODO change number of main loops?
        #                             pb_inst[racoon]['data'] = foundInd - deleteCount
        #                         else:
        #                             pass #TODO implement
        #                     else: # there is no end loop currently in the instruction. simply replace it.
        #                         # print('here1') 
        #                         pb_inst[racoon]['instruction'] = 'END_LOOP'
        #                         pb_inst[racoon]['data'] = foundInd - deleteCount
        #                         deleteCount = deleteCountPrev
        #                     pb_inst[i]['instruction'] += ' INNER DELETE'
        #                     deleteCountPrev += 1

        #             elif found and innerLoop not in pb_inst[i]['loops'] and i < foundInd + self.loopStarts[innerLoop]['numLoops']*loopLen: #TODO??foundInd < i and 
        #                 # print('last', i)
        #                 pb_inst[i]['instruction'] += ' INNER DELETE'
        #                 deleteCountPrev += 1
        #     ## INSERT any remaining instructions
        #     for i, item in enumerate(pb_insert_inst):
        #         pb_inst = np.insert(pb_inst, item[0], item[1])
        #         for inst in pb_inst[item[0]+1:]:
        #             pass #if pb_inst[] == 'END LOOP' increment by one


        #     print("??? ", len(pb_inst), self.loopStarts[innerLoop]['numLoops'], (loopLen))
        #     # print('\n\n\n')
        #     # for i in range(len(pb_inst)):
        #     #     print(i, pb_inst[i])
        #     # print('\n\n\n')
        #     # for ind in insertInd:
        #     #     pb_inst = np.insert(pb_inst, ind, )
        ...
        #         loopLen = 0 
        #         edgeCaseA_bool = 0     
        #         for i in range(len(pb_inst)-4): #go through all pb_inst
        #             if outerLoop in pb_inst[i]['loops'] and not found: #change pb_inst at beginning of loop
        #                 found = True
        #                 foundInd = i
        #                 loopLen+=1
        #                 if pb_inst[i]['instruction'] == 'LOOP':
        #                     if pb_inst[i]['data'] == 1:
        #                         pb_inst[i]['data'] = self.loopStarts[outerLoop]['numLoops']
        #                         pb_inst[i+1]['instruction'] += 'CONTINUE' #= 'CONTINUE' #ejd 8/22= 'CONTINUE'
        #                         edgeCaseA_bool = 1
        #                         print(i, 'edge case A bool is TRUE!')
        #                     else:
        #                         pass
        #                         #TODO also raise ERROR if not implemented yet
        #                 else:
        #                     pb_inst[i]['instruction'] = 'LOOP'
        #                     pb_inst[i]['data'] = self.loopStarts[outerLoop]['numLoops']
        #             elif outerLoop in pb_inst[i]['loops'] and found:
        #                 loopLen += 1
        #                 # print('here ', loopLen)
        #             elif found and outerLoop not in pb_inst[i]['loops'] and foundInd < i <= foundInd + loopLen: #reached end of loop, need to do a bunch of stuff
        #                 if i == foundInd + loopLen: #racoon is the last index of the loop 
        #                     racoon = i-1 #-1 or -2?
        #                     # print("RACOON OUTER", pb_inst[racoon])
        #                     if pb_inst[racoon]['instruction'] == 'END_LOOP': # if there is already an end_loop there...
        #                         index = pb_inst[racoon]['data'] # go back to beginning of this loop
        #                         if pb_inst[index]['data'] == 1: # if num loops ==1
        #                             pb_inst[index]['instruction'] = 'CONTINUE' #RACOON CONTINUE #change loop to continue
        #                             for fix in range(index,racoon):
        #                                 pb_inst[fix]['instruction'].replace(' RACOON OUTER DELETE',"") #TODO change number of main loops?
        #                             pb_inst[racoon]['data'] = foundInd - deleteCount
        #                         else:
        #                             pass #TODO implement
        #                     else: # there is no end loop currently in the instruction. simply replace it. 
        #                         pb_inst[racoon]['instruction'] += 'END_LOOP' #= 'END_LOOP' #ejd 8/22
        #                         pb_inst[racoon]['data'] = foundInd - deleteCount
        #                         deleteCount = deleteCountPrev
        #                     # check for EDGE CASE A
        #                     if pb_inst[i]['instruction'] == 'CONTINUE' and edgeCaseA_bool:
        #                         print(i, 'EDGE CASE A!')
        #                         loopLen = loopLen - 1
        #                     pb_inst[i]['instruction'] += ' OUTER DELETE'
        #                     deleteCountPrev += 1
                    
        #             elif found and outerLoop not in pb_inst[i]['loops'] and foundInd < i and i < foundInd + (self.loopStarts[outerLoop]['numLoops'])*loopLen+1: #TODO =1 here??
        #                 pb_inst[i]['instruction'] += str(i) + str(foundInd) +'X OUTER DELETE'
        #                 # if i + 1 == foundInd + self.loopStarts[outerLoop]['numLoops']*(1+loopLen):
        #                 #     pb_inst[i]['instruction'] = 'END_LOOP'
        #                 #     pb_inst[i]['data'] = foundInd
        #         print("??? ", len(pb_inst), self.loopStarts[outerLoop]['numLoops'], (loopLen), 7+self.loopStarts[outerLoop]['numLoops']*(loopLen))
