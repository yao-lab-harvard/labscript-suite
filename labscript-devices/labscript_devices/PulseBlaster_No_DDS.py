...
class PulseBlaster_No_DDS(PulseBlaster):
    ...
    def write_pb_inst_to_h5(self, pb_inst, hdf5_file):
        ...
        pb_META_dtype= [('loops',np.int32), ('outer_loop', np.int32)]
        pb_inst_table = np.empty(len(pb_inst),dtype = pb_dtype)
        pb_META_table = np.empty(1, dtype=pb_META_dtype)
        loops = self.loop_number
        outerLoop = self.outer_loop
  
        flagList = []
        instStr = ''
        delayList = []
        ####### offset_instructions_from_trigger = '' #TODO what?!?!?!
        for i,inst in enumerate(pb_inst):
            ...
            pb_META_table[0] = (self.loop_number, self.outer_loop)
        
        # # ejd 9/7/23
        # if outerLoop != 0: #TODO ejd do this in transition to buffered...?
        #     print('in outerLoop loop of pulseblaster_no_dds file')
        #     loop_table = np.empty(1,dtype = pb_dtype)
        #     loop_table[0] = (0,3,3,10) #TODO make general #,0,0,0,0,0,0,1)#(np.int32(extra_flags), np.int32(extra_inst),  np.int32(extra_inst_data),  np.float32(extra_length),   np.int32(loops), 0, 0, 0, 0, np.int32(additional_inst), np.int32(loop_start) ) #ejd 8/11/23
        #     # print("loop table:")
        #     # print(loop_table)
        #     pb_inst_table = np.insert(pb_inst_table, -3, loop_table) #ejd add 8/11/23
        #     # print(self.loop_number)
        #     pb_inst_table[3][2] = self.loop_number
        #     pb_inst_table[3][1] = 2
        #     pb_inst_table[-2][2] = len(pb_inst_table) - 3
        for i in range(len(pb_inst_table)): #usually printing
            print(i, pb_inst_table[i], '\n')
        ...
        group.create_dataset('PULSE_PROGRAM_METAINFO', compression=config.compression,data=pb_META_table)      
...
class PulseblasterNoDDSWorker(Worker):
    ...
    def transition_to_buffered(self,device_name,h5file,initial_values,fresh):
        # self.logger.info('pb transition to buffered')
        self.h5file = h5file
        if self.programming_scheme == 'pb_stop_programming/STOP':
        ...
        with h5py.File(h5file,'r') as hdf5_file:
            ...
            # pulse_METAINFO = group['PULSE_PROGRAM_METAINFO'][0]
            # loop_number = pulse_METAINFO[0]
            # outer_loop = pulse_METAINFO[1]

            if pulse_program[-3][0] > 8: #TO MAKE FPGA NOT DOUBLE TRIGGER #TODO what is this?
                pulse_program[-3][0] += -8

            #Let's get the final state of the pulseblaster:
            flags, _, _, _ = pulse_program[-1] 
            ...
            if fresh or (self.smart_cache['initial_values'] != initial_values) or \
                ...
                (self.smart_cache['pulse_program'] != pulse_program).any():
                    ...
                    for args in pulse_program:
                        # self.logger.info('line444 ' + str(args))
            ...
            # self.logger.info('pb exiting ttb')
...
#         loop_table = np.empty(1,dtype = pb_dtype)
#         loop_table[0] = (0,3,3,10,0,0,0,0,0,0,1)#(np.int32(extra_flags), np.int32(extra_inst),  np.int32(extra_inst_data),  np.float32(extra_length),   np.int32(loops), 0, 0, 0, 0, np.int32(additional_inst), np.int32(loop_start) ) #ejd 8/11/23
#         print("loop table:")
#         print(loop_table)
#         pb_inst_table = np.insert(pb_inst_table, -3, loop_table) #ejd add 8/11/23
#         # pb_inst_table = np.append(pb_inst_table, loop_table) #ejd 8/11/23
#         # pb_inst_table = np.append(pb_inst_table[0:self.inst_location], np.roll(pb_inst_table[len(pb_inst_table)-self.additional_inst-4:len(pb_inst_table) + 1],1)) #ejd 8/11/23
...
#                 #self.logger.info(i)
#                 pulse_program[i][1] = 0
#             loop_start = pulse_program[0][-1] #ejd 8/11/23
#             self.logger.info([loop_start])
#             self.logger.info("before:" + str(pulse_program))
#             #loop_start = 0 
#             pulse_program[loop_start][1] = 2
#             pulse_program[loop_start][2] = pulse_program[0][4]
#             self.logger.info("after:" + str(pulse_program))
...
#                 not self.smart_cache['ready_to_go']:
#                 # Enter programming mode
#                 self.logger.info('programming pb')
#                 pb_start_programming(PULSE_PROGRAM)
...
#                         self.logger.info('looping through args')
#                         pb_inst_pbonly(args[0], args[1], args[2],args[3])
                        
#                 if self.programming_scheme == 'pb_start/BRANCH':
#                     # We will be triggered by pb_start() if we are are the master pseudoclock or a single hardware trigger
#                     # from the master if we are not:
#                     self.logger.info('stop programming branch')
#                     pb_stop_programming()
#                 elif self.programming_scheme == 'pb_stop_programming/STOP':
#                     # Don't call pb_stop_programming(). We don't want to pulseblaster to respond to hardware
#                     # triggers (such as 50/60Hz line triggers) until we are ready to run.
