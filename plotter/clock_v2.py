...
class PageTwo(tk.Frame):
    ...
    def calculate_newest(self):
        directory = 'C:/Users/Yao Lab/labscript-suite/Experiments/hBN2/scan_Sept5'
        ...
        with h5py.File(h5_path,'r') as hdf5_file:
            parameters = hdf5_file['globals/scan_params']
            self.b_x_min = np.float64(parameters.attrs['Vx_min'])
            self.b_x_max = np.float64(parameters.attrs['Vx_max'])
            self.b_x_offset = np.float64(parameters.attrs['Vx_offset'])
            self.b_y_min = np.float64(parameters.attrs['Vy_min'])
            self.b_y_max = np.float64(parameters.attrs['Vy_max'])
            self.b_y_offset = np.float64(parameters.attrs['Vy_offset'])'''
        for file in os.listdir(newest_path):
            if file[-2:] == 'h5':
                h5File = h5py.File(newest_path+'\\'+file, "r")
                data = np.array(h5File['data/rawCounterData'])
            else:
                pass
        print(h5File)
        rasterParams = h5File['globals/scan_params'].attrs
        self.VxMin, self.VxMax, self.Vx0 = float(rasterParams['Vx_min']), float(rasterParams['Vx_max']), float(rasterParams['Vx_offset'])
        self.VyMin, self.VyMax, self.Vy0 = float(rasterParams['Vy_min']), float(rasterParams['Vy_max']), float(rasterParams['Vy_offset'])
        self.xpts, self.ypts = int(rasterParams['xPoints']), int(rasterParams['yPoints']) 
        self.dataDiffIm = np.append(np.diff(data), 0).reshape((self.xpts,self.ypts)).transpose()
        np.savetxt('colorplot.txt', self.dataDiffIm[int(self.xpts/2):,:])
    def plot_newest(self):
        ...
        V_x_units = self.Vx0 + np.linspace(self.VxMin, self.VxMax, self.xpts)
        V_y_units = self.Vy0 + np.linspace(self.VyMin, self.VyMax, int(self.ypts/2))
        V_x_mesh, V_y_mesh = np.meshgrid(V_x_units, V_y_units)
        print(np.shape(V_x_mesh), np.shape(V_y_mesh), np.shape(arr))
        #print(V_x_units)
    ...
    def plot_loaded(self):
        ...
        self.n = int( (len(arr))**0.5 )
        arr = np.reshape(arr, (self.n, self.n))
        arr[1::2, :] = arr[1::2, ::-1]
        np.savetxt('load_colorplot.txt', np.rot90(arr, 1))
        arr = np.loadtxt('load_colorplot.txt')
        ...
        with h5py.File(h5_path,'r') as hdf5_file:
            parameters = hdf5_file['globals/scan_parameters_non_blacs']
